...
describe('scav-ai alerts', () => {
  it('alertNearbyScavs attacks scavs that see the player', () => {
    const player = { dead: false, x: 500, y: 500 };
    const scavs = [
      {
        dead: false,
        x: 100,
    ...
    alertNearbyScavs(scavs, 110, 110, 200, 500, 500, player, []);
    expect(scavs[0].state).toBe('attack');
    expect(scavs[0].fireCooldown).toBeLessThanOrEqual(0.1);
  });

  it('alertNearbyScavs investigates when scav cannot see the player', () => {
    const player = { dead: false, x: 500, y: 500 };
    const scavs = [
      {
        dead: false,
